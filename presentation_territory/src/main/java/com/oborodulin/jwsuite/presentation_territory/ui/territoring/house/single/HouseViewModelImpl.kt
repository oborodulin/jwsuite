...
class HouseViewModelImpl @Inject constructor(
    ...
    DialogSingleViewModel<TerritoryStreetUiModel, UiState<TerritoryStreetUiModel>, HouseUiAction, UiSingleEvent, HouseFields, InputWrapper>(
        state, HouseFields.TERRITORY_STREET_ID.name,
        HouseFields.TERRITORY_STREET_TERRITORY
    ) {
    val localityId: StateFlow<InputWrapper> by lazy {
        state.getStateFlow(HouseFields.TERRITORY_STREET_IS_EVEN_SIDE.name, InputWrapper())
    }

    override val territory: StateFlow<InputListItemWrapper<ListItemModel>> by lazy {
        state.getStateFlow(
            HouseFields.TERRITORY_STREET_TERRITORY.name, InputListItemWrapper()
        )
    }
    override val street: StateFlow<InputListItemWrapper<ListItemModel>> by lazy {
        state.getStateFlow(
            HouseFields.TERRITORY_STREET_STREET.name, InputListItemWrapper()
        )
    }
    override val isPrivateSector: StateFlow<InputWrapper> by lazy {
        state.getStateFlow(
            HouseFields.TERRITORY_STREET_IS_PRIVATE_SECTOR.name, InputWrapper()
        )
    }
    ...
    override val estimatedHouses: StateFlow<InputWrapper> by lazy {
        state.getStateFlow(HouseFields.TERRITORY_STREET_EST_HOUSES.name, InputWrapper())
    }
    ...
    override fun initFieldStatesByUiModel(uiModel: TerritoryStreetUiModel): Job? {
        ...
        uiModel.id?.let {
            initStateValue(
                HouseFields.TERRITORY_STREET_ID, id, it.toString()
            )
        }
        initStateValue(
            HouseFields.TERRITORY_STREET_TERRITORY, territory,
            ListItemModel(
        ...
        initStateValue(
            HouseFields.TERRITORY_STREET_STREET, street,
            ListItemModel(
        ...
        initStateValue(
            HouseFields.TERRITORY_STREET_IS_PRIVATE_SECTOR, isPrivateSector,
            uiModel.territoryStreet.isPrivateSector.toString()
        )
        initStateValue(
            HouseFields.TERRITORY_STREET_IS_EVEN_SIDE, isEvenSide,
            uiModel.territoryStreet.isEvenSide.toString()
        )
        initStateValue(
            HouseFields.TERRITORY_STREET_EST_HOUSES, estimatedHouses,
        ...
        inputEvents.receiveAsFlow()
                ...
                when (event) {
                    is HouseInputEvent.Street ->
                        when (HouseInputValidator.Street.errorIdOrNull(event.input.headline)) {
                            null -> setStateValue(
                                HouseFields.TERRITORY_STREET_STREET, street, event.input,
                                true
                            )
                    ...
                    is HouseInputEvent.IsPrivateSector ->
                        setStateValue(
                            HouseFields.TERRITORY_STREET_IS_PRIVATE_SECTOR,
            ...
            .collect { event ->
                when (event) {
                    ...
                    is HouseInputEvent.IsEvenSide ->
                        setStateValue(
                            HouseFields.TERRITORY_STREET_IS_EVEN_SIDE, isEvenSide, null
                        )

                    is HouseInputEvent.EstHouses ->
                        setStateValue(
                            HouseFields.TERRITORY_STREET_EST_HOUSES, estimatedHouses, null
                        )
    ...
    override fun getInputErrorsOrNull(): List<InputError>? {
        ...
        HouseInputValidator.Street.errorIdOrNull(street.value.item?.headline)?.let {
            inputErrors.add(
                InputError(
                    fieldName = HouseFields.TERRITORY_STREET_STREET.name, errorId = it
                )
            )
        }
    ...
    override fun displayInputErrors(inputErrors: List<InputError>) {
        ...
        for (error in inputErrors) {
            state[error.fieldName] = when (HouseFields.valueOf(error.fieldName)) {
                HouseFields.TERRITORY_STREET_STREET -> street.value.copy(
