...
sealed class TerritoryStreetInputValidator : Validatable {
    data object Street : TerritoryStreetInputValidator() {
        override fun errorIdOrNull(vararg inputs: String?): Int? =
            when {
                inputs[0].isNullOrEmpty() -> com.oborodulin.jwsuite.presentation_geo.R.string.street_empty_error
                else -> null
            }
    }
    ...
    data object EstHouses : TerritoryStreetInputValidator() {
        override fun errorIdOrNull(vararg inputs: String?): Int? {
            ...
            return when {
                isNeedAddEstHouses == true && inputs[0].isNullOrEmpty() -> R.string.territory_street_est_houses_empty_error
                else -> null
            }
        }
    }
