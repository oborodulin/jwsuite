...
@HiltViewModel
class TransferObjectsListViewModelImpl @Inject constructor(
    private val useCases: RoleUseCases,
    private val converter: RolesListConverter
) : TransferObjectsListViewModel,
    ListViewModel<List<RolesListItem>, UiState<List<RolesListItem>>, TransferObjectsListUiAction, UiSingleEvent>() {

    override fun initState() = UiState.Loading

    override suspend fun handleAction(action: TransferObjectsListUiAction): Job {
        if (LOG_FLOW_ACTION) {
            Timber.tag(TAG).d("handleAction(RolesListUiAction) called: %s", action.javaClass.name)
        }
        val job = when (action) {
            is TransferObjectsListUiAction.Load -> loadRoles(action.memberId)
        }
        return job
    }
    ...
    companion object {
        fun previewModel(ctx: Context) =
            object : TransferObjectsListViewModel {
                ...
                override fun submitAction(action: TransferObjectsListUiAction): Job? = null
            }
