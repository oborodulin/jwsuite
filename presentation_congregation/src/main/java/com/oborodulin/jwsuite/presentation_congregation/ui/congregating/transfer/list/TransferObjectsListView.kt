...
@Composable
fun RolesListView(
    viewModel: TransferObjectsListViewModelImpl = hiltViewModel(),
    ...
    LaunchedEffect(memberInput?.memberId) {
        ...
        viewModel.submitAction(TransferObjectsListUiAction.Load(memberInput?.memberId))
    }
    ...
    LaunchedEffect(Unit) {
        Timber.tag(TAG)
            .d("RolesListView -> LaunchedEffect() AFTER collect single Event Flow")
