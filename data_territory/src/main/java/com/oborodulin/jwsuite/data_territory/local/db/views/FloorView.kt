...
FROM ${FloorEntity.TABLE_NAME} f JOIN ${HouseEntity.TABLE_NAME} h ON h.houseId = f.fHousesId 
    ...
    LEFT JOIN ${TerritoryView.VIEW_NAME} tv ON tv.territoryId = f.fTerritoriesId AND tv.${TerritoryEntity.PX_LOCALITY}localityLocCode  = sv.streetLocCode
"""
)
class FloorView(
...
/*
    ...
    @Embedded(prefix = HouseEntity.PX_LD_REGION) val fldRegion: GeoRegionView?,
    @Embedded(prefix = HouseEntity.PX_LD_REGION_DISTRICT) val fldDistrict: RegionDistrictView?,
    @Embedded(prefix = HouseEntity.PX_LD_LOCALITY) val fldLocality: LocalityView?,

    @Embedded(prefix = HouseEntity.PX_M_REGION) val fmRegion: GeoRegionView?,
    @Embedded(prefix = HouseEntity.PX_M_REGION_DISTRICT) val fmDistrict: RegionDistrictView?,
    @Embedded(prefix = HouseEntity.PX_M_LOCALITY) val fmLocality: LocalityView?,
    @Embedded(prefix = HouseEntity.PX_M_LOCALITY_DISTRICT) val fmLocalityDistrict: LocalityDistrictView?,
*/