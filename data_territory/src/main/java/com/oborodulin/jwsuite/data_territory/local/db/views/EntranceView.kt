...
FROM ${EntranceEntity.TABLE_NAME} e JOIN ${HouseEntity.TABLE_NAME} h ON h.houseId = e.eHousesId 
    ...
    LEFT JOIN ${TerritoryView.VIEW_NAME} tv ON tv.territoryId = e.eTerritoriesId AND tv.${TerritoryEntity.PX_LOCALITY}localityLocCode  = sv.streetLocCode
"""
)
class EntranceView(
...
/*
    ...
    @Embedded(prefix = HouseEntity.PX_LD_REGION) val eldRegion: GeoRegionView?,
    @Embedded(prefix = HouseEntity.PX_LD_REGION_DISTRICT) val eldDistrict: RegionDistrictView?,
    @Embedded(prefix = HouseEntity.PX_LD_LOCALITY) val eldLocality: LocalityView?,
 
    @Embedded(prefix = HouseEntity.PX_M_REGION) val emRegion: GeoRegionView?,
    @Embedded(prefix = HouseEntity.PX_M_REGION_DISTRICT) val emDistrict: RegionDistrictView?,
    @Embedded(prefix = HouseEntity.PX_M_LOCALITY) val emLocality: LocalityView?,
    @Embedded(prefix = HouseEntity.PX_M_LOCALITY_DISTRICT) val emLocalityDistrict: LocalityDistrictView?,
*/