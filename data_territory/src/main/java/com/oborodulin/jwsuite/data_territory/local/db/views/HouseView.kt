...
SELECT sv.*,
        ...
        ldv.localityDistrictId AS ${HouseEntity.PX_LOCALITY_DISTRICT}localityDistrictId, ldv.locDistrictShortName AS ${HouseEntity.PX_LOCALITY_DISTRICT}locDistrictShortName, 
            ldv.locDistrictGeocode AS ${HouseEntity.PX_LOCALITY_DISTRICT}locDistrictGeocode, ldv.locDistrictOsmId AS ${HouseEntity.PX_LOCALITY_DISTRICT}locDistrictOsmId, 
            ldv.${GeoLocalityDistrictEntity.PREFIX}latitude AS ${HouseEntity.PX_LOCALITY_DISTRICT}${GeoLocalityDistrictEntity.PREFIX}latitude,
            ldv.${GeoLocalityDistrictEntity.PREFIX}longitude AS ${HouseEntity.PX_LOCALITY_DISTRICT}${GeoLocalityDistrictEntity.PREFIX}longitude, 
            ldv.ldLocalitiesId AS ${HouseEntity.PX_LOCALITY_DISTRICT}ldLocalitiesId, ldv.localityDistrictTlId AS ${HouseEntity.PX_LOCALITY_DISTRICT}localityDistrictTlId, 
            ldv.locDistrictLocCode AS ${HouseEntity.PX_LOCALITY_DISTRICT}locDistrictLocCode, ldv.locDistrictName AS ${HouseEntity.PX_LOCALITY_DISTRICT}locDistrictName, 
            ldv.localityDistrictsId AS ${HouseEntity.PX_LOCALITY_DISTRICT}localityDistrictsId, 
        ...
        tv.*, h.*
FROM ${HouseEntity.TABLE_NAME} h JOIN ${StreetView.VIEW_NAME} sv ON sv.streetId = h.hStreetsId
    ...
    LEFT JOIN ${TerritoryView.VIEW_NAME} tv ON tv.territoryId = h.hTerritoriesId AND tv.${TerritoryEntity.PX_LOCALITY}localityLocCode  = sv.streetLocCode
"""
)
class HouseView(
    @Embedded val street: StreetView,
    @Embedded(prefix = HouseEntity.PX_LOCALITY) val locality: LocalityView,
    @Embedded(prefix = HouseEntity.PX_LOCALITY_DISTRICT) val localityDistrict: LocalityDistrictView?,
...
/*
    ...
    @Embedded(prefix = HouseEntity.PX_LD_REGION) val hldRegion: GeoRegionView?,
    @Embedded(prefix = HouseEntity.PX_LD_REGION_DISTRICT) val hldDistrict: RegionDistrictView?,
    @Embedded(prefix = HouseEntity.PX_LD_LOCALITY) val hldLocality: LocalityView?,

    @Embedded(prefix = HouseEntity.PX_M_REGION) val hmRegion: GeoRegionView?,
    @Embedded(prefix = HouseEntity.PX_M_REGION_DISTRICT) val hmDistrict: RegionDistrictView?,
    @Embedded(prefix = HouseEntity.PX_M_LOCALITY) val hmLocality: LocalityView?,
    @Embedded(prefix = HouseEntity.PX_M_LOCALITY_DISTRICT) val hmLocalityDistrict: LocalityDistrictView?,
 */