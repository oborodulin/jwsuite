...
data class GeoRegionTlEntity(
    ...
    companion object {
        ...
        fun regionTl(ctx: Context, regionCode: String, regionId: UUID) =
            when (regionCode) {
                ctx.resources.getString(R.string.def_reg_donetsk_code) -> donetskRegionTl(ctx, regionId)
                ctx.resources.getString(R.string.def_reg_luhansk_code) -> luganskRegionTl(ctx, regionId)
