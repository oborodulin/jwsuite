...
import com.oborodulin.jwsuite.data.util.Constants.PX_HOUSE_LD_REGION
import com.oborodulin.jwsuite.data.util.Constants.PX_HOUSE_LD_REGION_DISTRICT
import com.oborodulin.jwsuite.data.util.Constants.PX_HOUSE_MICRODISTRICT
import com.oborodulin.jwsuite.data.util.Constants.PX_HOUSE_M_LOCALITY
import com.oborodulin.jwsuite.data.util.Constants.PX_HOUSE_M_LOCALITY_DISTRICT
...
SELECT sv.*,
        rvl.regionId AS ${PX_HOUSE_LD_REGION}regionId, rvl.regionCode AS ${PX_HOUSE_LD_REGION}regionCode, 
            rvl.regionTlId AS ${PX_HOUSE_LD_REGION}regionTlId, rvl.regionLocCode AS ${PX_HOUSE_LD_REGION}regionLocCode, rvl.regionName AS ${PX_HOUSE_LD_REGION}regionName,
            rvl.regionsId AS ${PX_HOUSE_LD_REGION}regionsId, 
        ...
        rvm.regionId AS ${PX_HOUSE_M_REGION}regionId, rvm.regionCode AS ${PX_HOUSE_M_REGION}regionCode, 
            rvm.regionTlId AS ${PX_HOUSE_M_REGION}regionTlId, rvm.regionLocCode AS ${PX_HOUSE_M_REGION}regionLocCode, rvm.regionName AS ${PX_HOUSE_M_REGION}regionName,
            rvm.regionsId AS ${PX_HOUSE_M_REGION}regionsId, 
...
FROM ${HouseEntity.TABLE_NAME} h JOIN ${GeoStreetView.VIEW_NAME} sv ON sv.streetId = h.hStreetsId
    ...
    LEFT JOIN ${TerritoryView.VIEW_NAME} tv ON tv.territoryId = h.hTerritoriesId AND tv.${PX_LOCALITY}localityLocCode  = sv.streetLocCode
"""
)
class HouseView(
    @Embedded val street: GeoStreetView,

    @Embedded(prefix = PX_HOUSE_LD_REGION) val ldRegion: GeoRegionView?,
    @Embedded(prefix = PX_HOUSE_LD_REGION_DISTRICT) val ldDistrict: RegionDistrictView?,
    @Embedded(prefix = PX_HOUSE_LD_LOCALITY) val ldLocality: LocalityView?,
    @Embedded(prefix = PX_HOUSE_LD_LOCALITY_DISTRICT) val localityDistrict: LocalityDistrictView?,

    @Embedded(prefix = PX_HOUSE_M_REGION) val mRegion: GeoRegionView?,
    @Embedded(prefix = PX_HOUSE_M_REGION_DISTRICT) val mDistrict: RegionDistrictView?,
    @Embedded(prefix = PX_HOUSE_M_LOCALITY) val mLocality: LocalityView?,
    @Embedded(prefix = PX_HOUSE_M_LOCALITY_DISTRICT) val mLocalityDistrict: LocalityDistrictView?,
    @Embedded(prefix = PX_HOUSE_MICRODISTRICT) val microdistrict: MicrodistrictView?,
