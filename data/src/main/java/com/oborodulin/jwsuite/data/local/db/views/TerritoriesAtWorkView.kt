...
@DatabaseView(
    ...
    value = """
SELECT t.*, m.*, ct.ctCongregationsId, tps.isPrivateSector, (rld.fullAtHandDays - rld.territoryAtHandPeriod * 30) AS expiredDays
FROM ${TerritoryView.VIEW_NAME} t JOIN ${CongregationTerritoryCrossRefEntity.TABLE_NAME} ct 
    ...
    JOIN ${TerritoryMemberLastReceivingDateView.VIEW_NAME} rld ON rld.territoryId = t.territoryId AND rld.fullIdleMonths < 0
    JOIN ${MemberView.VIEW_NAME} m ON m.memberId = rld.tmcMembersId
ORDER BY (rld.fullAtHandDays - rld.territoryAtHandPeriod * 30) DESC
"""
)
