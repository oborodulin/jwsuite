    ...
    override suspend fun handleAction(action: MicrodistrictUiAction): Job {
        ...
        val job = when (action) {
            is MicrodistrictUiAction.Load -> when (action.microdistrictId) {
                null -> {
                    setDialogTitleResId(com.oborodulin.jwsuite.presentation.R.string.microdistrict_new_subheader)
                    submitState(UiState.Success(MicrodistrictUi()))
                }

                else -> {
                    setDialogTitleResId(com.oborodulin.jwsuite.presentation.R.string.microdistrict_subheader)
