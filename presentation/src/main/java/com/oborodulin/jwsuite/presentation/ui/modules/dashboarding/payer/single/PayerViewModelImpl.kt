package com.oborodulin.jwsuite.presentation.ui.modules.dashboarding.payer.single
...
class PayerViewModelImpl @Inject constructor(
    ...
    SingleViewModel<CongregationUi, UiState<CongregationUi>, PayerUiAction, UiSingleEvent, PayerFields, InputWrapper>(
        state,
    ...
    override fun initFieldStatesByUiModel(uiModel: Any): Job? {
        ...
        initStateValue(PayerFields.ERC_CODE, ercCode, congregationUi.congregationNum)
        initStateValue(PayerFields.FULL_NAME, fullName, congregationUi.congregationName)
        initStateValue(PayerFields.ADDRESS, address, congregationUi.territoryMark)
        initStateValue(PayerFields.TOTAL_AREA, totalArea, congregationUi.totalArea?.toString() ?: "")
        initStateValue(
        ...
        initStateValue(PayerFields.PAYMENT_DAY, paymentDay, congregationUi.paymentDay.toString())
        initStateValue(PayerFields.PERSONS_NUM, personsNum, congregationUi.personsNum.toString())
        return null
    }
