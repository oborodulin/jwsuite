...
fun RegionDistrictsListView(
    ...
    LaunchedEffect(regionInput) {
        ...
        viewModel.submitAction(LocalityDistrictsListUiAction.Load(regionInput.regionId))
    }
            ...
            RegionDistrictsList(
                ...
                onEdit = { regionDistrict ->
                    viewModel.submitAction(
                        LocalityDistrictsListUiAction.EditLocalityDistrict(regionDistrict.id)
                    )
                },
                onDelete = { regionDistrict ->
                    viewModel.submitAction(
                        LocalityDistrictsListUiAction.DeleteLocalityDistrict(regionDistrict.id)
                    )
                }
            ) {}
        }
...
fun PreviewRegionDistrictsList() {
    JWSuiteTheme {
        Surface {
            RegionDistrictsList(
                regionDistricts = LocalityDistrictsListViewModelImpl.previewList(LocalContext.current),
                onEdit = {},
                onDelete = {},
                onClick = {})
        }
